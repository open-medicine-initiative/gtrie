      ...
      describe ('Prediction using sample dataset', () => {
        ...
        describe('Predictions in diverse population', () => {
          it('Symptom shared by many diseases leads to predcition of symptoms from disease with highest relative frequency', () => {
            var population = {
              "type one" : {times : 2, disease : "Aganglionic Megacolon"},
              "type two" : {times : 1, disease : "Bladder Infections in Children"},
              "type three" : {times : 1, disease : "Necrotizing Enterocolitis"},
              "type four" : {times : 1, disease :"Small bowel obstruction"}
            };
            var testTrie = new GTrie();
            new UserEmulator(testTrie).multipleUsers(population);
            var seq = testTrie.sequence().input("Vomiting.");
            expect(seq.predict().input).to.equal("Not wanting to eat.");// from symptoms of "Aganglionic Megacolon", as it possesses higher frequency in population
          });
        });
