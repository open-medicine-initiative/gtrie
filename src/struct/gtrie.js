...
class Node {
    ...
    predict(threshold = 1 / 2) {
        ...
        var candidates = this.out
            .map((link, input) => {return {
                prediction: input,
                relativeFrequency: link.counter / nodevisited}
            })
            .filter(x => x.relativeFrequency >= threshold)
            .sort((valueA, valueB) => {
        ...
        var prediction= new Predictionresult({prediction: 'no prediction', relativeFrequency: 0}, candidates);
        return prediction;
...
class Predictionresult{
    ...
    constructor(catcher, array) {
        this.catcher = catcher;
        this.array =  array;
    }

    getprediction(k) {
        return (this.array[k] !== undefined)?  this.array[k] : this.catcher;
    }
